...
import { fmtDayLong, fmtRange, joinUrl, durationMin, isCancelled } from "./bookingUtils";
  ...
  return (
    <div className="pip-view-in pb-32">
      <div className="flex items-center justify-between gap-2 mb-4">
        <button type="button" onClick={onBack} className="h-11 pl-2 pr-4 rounded-full inline-flex items-center gap-1 text-[16px] font-medium" style={{ background: "var(--crm-card-bg)", color: CRM.ink, boxShadow: "0 1px 6px rgba(45,35,32,0.10)", border: CRM.cardBorder }}>
        ...
        <div className="flex items-center gap-2">
          ...
          {url && !isCancelled(booking) && (
            <a href={url} target="_blank" rel="noopener noreferrer" className="h-11 px-5 rounded-full inline-flex items-center text-[16px] font-semibold" style={{ background: CRM.ink, color: "var(--crm-page-bg)" }}>
              Join
            </a>
          )}
        </div>
      ...
      <h1 className="text-[26px] font-bold leading-tight" style={{ color: CRM.ink }}>{booking.title || "Meeting"}</h1>
      <div className="mt-3 flex items-center gap-2 text-[13px]" style={{ color: CRM.sub }}>
      ...
      <p className="mt-3 text-[16px] leading-relaxed" style={{ color: CRM.ink }}>
      ...
      <section className="mt-6 rounded-2xl overflow-hidden" style={CARD}>
        <div className="flex items-center justify-between px-4 py-3.5" style={{ borderBottom: "1px solid rgba(182,118,81,0.10)" }}>
          <h2 className="text-[17px] font-semibold" style={{ color: CRM.ink }}>Participants</h2>
          <span className="text-[14px]" style={{ color: CRM.sub }}>{participantCount}</span>
        </div>
        {organizer && (
          <div className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid rgba(182,118,81,0.08)" }}>
            ...
            <span className="flex-1 text-[15px] truncate" style={{ color: CRM.ink }}>{organizer.name || organizer.email} <span style={{ color: CRM.sub }}>(Organizer)</span></span>
        ...
        {attendees.map((a) => (
          <div key={a.email} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid rgba(182,118,81,0.08)" }}>
            ...
            <span className="flex-1 text-[15px] truncate" style={{ color: CRM.ink }}>{a.name || a.email}</span>
        ...
        {guests.map((g) => (
          <div key={g} className="flex items-center gap-3 px-4 py-3" style={{ borderBottom: "1px solid rgba(182,118,81,0.08)" }}>
            ...
            <span className="flex-1 text-[15px] truncate" style={{ color: CRM.ink }}>{g} <span style={{ color: CRM.sub }}>(Guest)</span></span>
        ...
        {primaryEmail && (
          <div className="px-4 py-3 flex items-center gap-3">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[15px] truncate" style={{ color: CRM.ink }}>{primaryEmail}</p>
            </div>
        ...
        {lead && (
          <button type="button" onClick={() => onOpenLead(lead)} className="w-full flex items-center gap-3 px-4 py-3.5 text-left" style={{ borderTop: "1px solid rgba(182,118,81,0.10)" }}>
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[15px] font-medium truncate" style={{ color: CRM.ink }}>{displayName(lead)}</p>
            </div>
