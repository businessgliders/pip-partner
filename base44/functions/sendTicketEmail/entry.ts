...
<div style="margin-top:16px;padding:12px 14px;border:1px solid #e2e8f0;border-radius:8px;background:#f8fafc;">
  ...
  <ul style="margin:0;padding-left:18px;font-size:14px;color:#334155;">${items}</ul>
    ...
    // Wrap inner content in the branded shell so composer emails match template tests
    const wrappedBodyHtml = brandedShell(finalBodyHtml, subject);
    ...
    const mimeBody = [
      ...
      'Content-Transfer-Encoding: quoted-printable',
      '',
      quotedPrintable(wrappedBodyHtml),
      '',
      `--${boundary}--`,
      '',
    ].join('\r\n');
