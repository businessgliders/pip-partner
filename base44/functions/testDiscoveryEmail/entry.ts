// One-off test endpoint that sends the branded "Discovery Call Confirmed"
// applicant email to a chosen test recipient, using a real inquiry's data.
// Admin-only.
...
function submitterEmail(firstName, scheduledTime, appNumber) {
...
Deno.serve(async (req) => {
  try {
    ...
    const { to, firstName = 'Moe', scheduledTime = 'Tuesday, May 26, 2026 at 8:00 PM', appNumberRaw = 13 } =
      await req.json();
    if (!to) return Response.json({ error: 'Missing "to"' }, { status: 400 });

    const { accessToken } = await base44.asServiceRole.connectors.getConnection('gmail');
    const appNumber = formatAppNumber(appNumberRaw);
    const subject = `[TEST] [Application #${appNumber}] Your discovery call is confirmed \u2014 Pilates in Pink \u2122`;
    const html = submitterEmail(firstName, scheduledTime, appNumber);
    ...
    const mimeBody = [
      ...
      quotedPrintable(htmlToText(html)), '',
      `--${boundary}`,
    ...
    return Response.json({ success: true, to, subject, messageId: data.id });
  } catch (error) {
    console.error('testDiscoveryEmail error', error);
