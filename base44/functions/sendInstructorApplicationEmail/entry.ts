...
function buildViewButton(applicationId) {
  const boardUrl = `https://partner.pilatesinpinkstudio.com/ApplicationBoard?tab=instructor&ticket=${applicationId}`;
...
Deno.serve(async (req) => {
  try {
    ...
    const result = await sendGmail({
      accessToken,
      to: recipients,
      subject: `${subjectPrefix}New Instructor Application: ${safeName}`,
      html,
      replyTo: safeReplyTo,
    });
