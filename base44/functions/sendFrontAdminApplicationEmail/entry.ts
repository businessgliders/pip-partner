...
function buildViewButton(applicationId) {
  const boardUrl = `https://partner.pilatesinpinkstudio.com/ApplicationBoard?tab=frontadmin&ticket=${applicationId}`;
...
Deno.serve(async (req) => {
  try {
    ...
    let applicationData;
    try {
      applicationData = await base44.entities.FrontAdminApplication.get(applicationId);
    } catch (_) {
    ...
    let recipients = OWNER_EMAILS.join(', ');
    ...
    const result = await sendGmail({
      accessToken,
      to: recipients,
      subject: `${subjectPrefix}New Front Desk Admin Application: ${safeName}`,
      html,
      replyTo: safeReplyTo,
    });
